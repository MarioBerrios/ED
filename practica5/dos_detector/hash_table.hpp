...
public:
    ...
    {
        //TODO
        return (indice_ < m_) && (it_ != tabla_[indice_].end());
    }
    ...
    size_t num_of_valid_keys() const
    {
		...
		for(size_t i = 0; i < m_; i++){
			if(tabla_[i].empty() == false){
				keys = keys + tabla_[i].size();
			}
		}
        return keys;
    }
    ...
    {
        //TODO
        return num_of_valid_keys()/m_*1.0;
    }
    ...
    bool has(K const& k) const
    {
		...
		for (size_t i = 0; i < m_; i++){
			if (!tabla_[i].empty()){
				for(auto j = tabla_[i].begin(); j != tabla_[i].end(); j++){
					if(j -> first == k)
						return true;
				}
			}
    ...
    bool find(K const& k)
    {
        ...
        typename std::list <std::pair<K, V>>::iterator aux_it = it_;
		size_t aux_indice = indice_;

		indice_ = hash(key_to_int_(k));
		if(tabla_[indice_].empty()){
			indice_ = aux_indice;
			return is_found;
		}

		it_ = tabla_[indice_].begin();
		while(it_ != tabla_[indice_].end()){
			if(get_key() == k){
				is_found = true;
				return is_found;
			}
    ...
    void insert(K const& k, V const& v)
    {
		...
		if(find(k) == true){
			...
			indice_ = hash(key_to_int_(k));
			tabla_[indice_].push_front(std::pair<K, V>(k, v));
			it_ = tabla_[indice_].begin();
    ...
    void remove()
    {
		...
		tabla_[aux_indice].erase(aux_it);
        //
...
protected:
	...
	std::vector<std::list<std::pair<K, V>>> tabla_;
	size_t indice_;
